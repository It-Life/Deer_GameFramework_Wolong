...
namespace Deer
{
    ...
    public abstract class ConfigBase<T> : IConfig where T : class, new()
    {
        ...
        {
            U config = default(U);
            HotfixFramework.Runtime.FileUtils.FileReadAllBytes(Utility.Text.Format("Config/{0}.bin", excelName), isReadWritePath, delegate (bool isRead, byte[] result)
            {
                //反序列化
        ...
        protected void AnalyseConfig(string excelName, bool isReadWritePath, Action<byte[]> action)
        {
            HotfixFramework.Runtime.FileUtils.FileReadAllBytes(Utility.Text.Format("Config/{0}.bin", excelName), isReadWritePath, delegate (bool isRead, byte[] result)
            {
                //反序列化
        ...
        public U Deserialize<U>(byte[] content, string excelName)
        {
            ...
            if (content.Length > 0)
            {
                try
                {
                    //ProtobufUtils.Deserialize<U>(content);
                    Logger.Info("Deserialize:"+ excelName);
