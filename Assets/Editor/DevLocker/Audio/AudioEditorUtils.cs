...
namespace DevLocker.Tools.Audio
{
	...
	public static class AudioEditorUtils
	{
		...
		public static int GetOrigSize(AudioImporter aimp) {
			var audioImporterType = aimp.GetType();
			
			var method = audioImporterType.GetProperty("origSize", BindingFlags.NonPublic | BindingFlags.Instance);
			if (method != null)
			{
				return (int) method.GetValue(aimp);
			}

			Debug.Log("origSize method not found on audio importer, size not available");
			return 0;
		}
	}
